...
const dayAgendaSections = computed(() => {
        ...
        const attendanceRequestItems = (dailyAttendanceRequests.data || []).map((row) => ({
                ...
                subtitle: `وضعیت: ${docstatusLabel(row.docstatus)}`,
                meta: "",
...
function docstatusLabel(value) {
        const ds = parseDocstatus(value)
        if (ds === 1) return "تایید شده"
        if (ds === 2) return "رد شده / لغو"
        return "در انتظار"
}
...
const monthLeaveApplications = createListResource({
        ...
        fields: ["name", "leave_type", "from_date", "to_date", "docstatus"],
        filters: {},
...
const dailyShiftRequests = createListResource({
        ...
        fields: ["name", "docstatus", "from_date", "to_date", "shift_type"],
        filters: {},
        orderBy: "modified desc",
        auto: false,
})
