                        ...
                        <div class="w-full max-w-5xl mx-auto flex flex-col items-center mt-7 mb-7 p-4">
                                ...
                                <div v-if="leaveCharts.length" class="w-full bg-white rounded p-4 my-2">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                                <div
                                                        v-for="chart in leaveCharts"
                                                        :key="chart.leaveType"
                                                        class="flex flex-col items-center gap-2 p-3 rounded-xl bg-gray-50"
                                                >
                                                        <div class="relative w-20 h-20">
                                                                <svg viewBox="0 0 100 100" class="w-20 h-20 -rotate-90">
                                                                        ...
                                                                        <circle
                                                                                ...
                                                                                :stroke-dasharray="`${chart.usedRatio * 226.19} 226.19`"
                                                                        />
                                                                </svg>
                                                                <div class="absolute inset-0 flex flex-col items-center justify-center">
                                                                        <span class="text-sm font-bold text-gray-800">{{ chart.balance }}</span>
                                                                        <span class="text-[9px] text-gray-400">مانده</span>
                                                                </div>
                                                        </div>
                                                        <div class="text-center">
                                                                <div class="text-xs font-medium text-gray-700 leading-tight">{{ chart.leaveType }}</div>
                                                                <div class="text-[10px] text-gray-400 mt-0.5">{{ chart.used }} از {{ chart.total }} روز</div>
                                                        </div>
                                                </div>
                                        </div>
                                </div>
...
const leaveCharts = computed(() => {
        const balanceMap = leaveBalanceResource.data || {}
        return Object.entries(balanceMap).map(([leaveType, details], index) => {
                const total = details.allocated_leaves || 0
                const balance = Math.max(0, details.balance_leaves || 0)
