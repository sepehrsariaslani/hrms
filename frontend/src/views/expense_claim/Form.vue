...
const postingDateFromQuery = computed(() => {
...
const pageTitle = computed(() =>
	props.id ? __("Expense Claim") : __("New Expense Claim")
)
...
const expenseClaim = ref({
	...
	posting_date: postingDateFromQuery.value,
})
...
const formFields = createResource({
	...
	transform(data) {
		...
		return fields.map((field) => {
			if (field.fieldname === "posting_date") field.default = postingDateFromQuery.value
...
watch(
	...
	(value) => {
		...
		expenseClaim.value.posting_date = queryDate
	},
	{ immediate: true }
)
watch(
