...
function setLeaveTypes(data) {
        ...
        const typeList = (data || []).map((typeName) => ({
                label: localizeLeaveType(typeName),
                value: typeName,
        }))
        leave_type.documentList = typeList
        // force Select fieldtype so only allocated leave types are shown
        leave_type.fieldtype = "Select"
        leave_type.options = (data || []).join("\n")
        if (!leaveApplication.value.leave_type && typeList.length) {
                leaveApplication.value.leave_type = typeList[0].value
        }
}
