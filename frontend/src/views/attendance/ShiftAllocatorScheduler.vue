                                                                ...
                                                                <div class="p-2 col-span-2 border-l border-gray-100">
                                                                        ...
                                                                        <div class="mt-1.5 flex flex-wrap gap-1">
                                                                                <button
                                                                                        ...
                                                                                        :key="`repeat-${employee.name}-${slotCode}`"
                                                                                        type="button"
                                                                                        class="rounded px-1.5 py-0.5 text-[10px] font-semibold border border-gray-300 bg-gray-50 hover:bg-gray-100 text-gray-700"
                                                                                        :title="'تکرار ' + (slotCode === 'Morning' ? 'صبح' : 'عصر') + ' برای همه روزهای هفته'"
                                                                                        @click.stop="repeatSlotAllWeek(employee, slotCode)"
                                                                                >
                                                                                        ↻ {{ slotCode === "Morning" ? "صبح همه" : "عصر همه" }}
                                                                                </button>
                                                                        </div>
                                                                </div>
                                                                <div
...
function repeatSlotAllWeek(employee, slotCode) {
        const availableDays = weekDays.value.filter((day) => !isForbidden(employee.name, day.date))
        const allAssigned = availableDays.every((day) => isAssigned(employee.name, day.date, slotCode))
        const currentKeys = new Set(assignedSlotsByEmployee.value[employee.name] || [])
        for (const day of availableDays) {
                const key = slotKey(day.date, slotCode)
                if (allAssigned) {
        ...
        assignedSlotsByEmployee.value = {
                ...assignedSlotsByEmployee.value,
                [employee.name]: [...currentKeys].sort(),
        }
}
