                        ...
                        <div class="flex flex-col p-4 gap-4 mb-8">
                                ...
                                <div v-if="isEditable" class="flex flex-col gap-2">
                                        <Button
                                                variant="outline"
                                                class="w-full"
                                                :disabled="!isEditable || copyingPreviousWeek"
                                                :loading="copyingPreviousWeek"
                                                @click="copyFromPreviousWeek"
                                        >
                                                کپی شیفت‌ها از هفته قبل
                                        </Button>
...
const prevWeekContextResource = createResource({
        url: "hrms.api.weekly_shift.get_weekly_shift_context",
        auto: false,
})
...
async function copyFromPreviousWeek() {
        ...
        try {
                const prevWeekStart = addDays(weekStartDate.value, -7)
                const data = await prevWeekContextResource.fetch({
                        week_start_date: prevWeekStart,
                        employee: selectedEmployee.value || undefined,
                })
                const prevSlots = data?.plan?.slot_details || []
                if (!prevSlots.length) {
                        showSuccessToast("هفته قبل هیچ شیفتی ثبت نشده بود")
                        return
                }
                let copiedCount = 0
                for (const slot of prevSlots) {
                        if (!(slot?.work_date && slot?.slot_code)) continue
                        const prevDayOfWeek = new Date(`${slot.work_date}T12:00:00`).getDay()
                        const currentDay = weekDays.value.find(
                                (day) => new Date(`${day.date}T12:00:00`).getDay() === prevDayOfWeek
                        )
                        if (!currentDay) continue
                        const currentDate = currentDay.date
                        if (isForbiddenDate(currentDate)) continue
                        if (!getEditableSlot(currentDate, slot.slot_code)) {
                                toggleSlot(currentDate, slot.slot_code, true)
                                const addedSlot = getEditableSlot(currentDate, slot.slot_code)
                                if (addedSlot && slot.duty_role) {
                                        addedSlot.duty_role = slot.duty_role
                                }
                                copiedCount++
                        }
                }
                if (copiedCount > 0) {
                        showSuccessToast(`${copiedCount} شیفت از هفته قبل کپی شد`)
                } else {
                        showSuccessToast("همه شیفت‌های هفته قبل قبلاً انتخاب شده‌اند")
                }
        } catch {
