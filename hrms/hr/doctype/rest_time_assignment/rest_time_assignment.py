...
class RestTimeAssignment(Document):
	def validate(self):
	...
	def validate_overlapping_assignments(self):
		...
		overlap = frappe.db.sql(
			"""
			SELECT name
			FROM `tabRest Time Assignment`
			WHERE employee = %(employee)s
			  AND rest_time = %(rest_time)s
			  AND docstatus = 1
			  AND status = 'Active'
			  AND name != %(name)s
		...
		if overlap:
			frappe.throw(
				_("Employee {0} already has an active assignment for {1}: {2}").format(
					...
					frappe.bold(self.rest_time),
					get_link_to_form("Rest Time Assignment", overlap[0].name),
				),
				title=_("Overlapping Rest Time Assignment"),
			)
